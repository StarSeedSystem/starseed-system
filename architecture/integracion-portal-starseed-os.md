...
## Trinity · Bloque 5 — Motor de gestos unificado y cierre común (v3 · 2026-09-25)

> Alex: «ni el arrastre para abrir o cerrar los menús de los lados ni el botón de cierre funcionan en los del menú Trinidad… tampoco son estéticos».

### Causa real (reproducida en Chromium sin GPU con ratón y con toques CDP, antes de tocar nada)
1. **La X y todos los botones de dentro no respondían con ratón.** La capa de arrastre de cada cortina llamaba a `setPointerCapture` en **cada** `pointerdown` (también el de la X, que burbujeaba hasta ella). Con el puntero capturado por la capa, Chromium entrega el `click` a la capa y no al botón pulsado. Afectaba también a los botones del Exocórtex y del Centro de Creación. Las pruebas de jsdom no lo veían (`setPointerCapture` no existe allí y el `click` se disparaba a mano).
2. **Arrastrar no hacía nada en táctil.** Las cortinas laterales no declaraban `touch-action`, así que el navegador se quedaba el deslizamiento horizontal y lanzaba `pointercancel`. Incluso con `pan-y` en el panel, cada contenedor con scroll interior vuelve a empezar la cuenta de `touch-action`, y Chrome llegó a convertir el gesto en «atrás» del historial (la página navegó a `about:blank`).
3. **Los tiradores laterales quedaban debajo de los sensores de borde** (`perimeter-interface.tsx`, `z-[9999]`): el toque caía en el sensor, que alternaba la cortina en vez de arrastrarla.
4. En Logic de escritorio la X flotaba fuera del panel (en la esquina del contenedor de altura completa) y la X interna del Centro de Control era `md:hidden`.
5. **El cromo Trinity táctil estaba oculto siempre.** `ui/dialog` registraba el «modal a pantalla completa» (Adenda 188) en un efecto de `DialogContent`, que corre aunque el diálogo esté cerrado; el diálogo de `usePrompt` (ConfirmProvider, montado siempre en el layout raíz) dejaba `body[data-ss-modal="1"]` puesto para siempre y globals.css escondía las asas de borde, el botón Trinity y el acceso a la guía en todo el OS (y los popups de primera ejecución esperaban un primer plano libre que no llegaba). Ahora el registro vive DENTRO de `DialogPrimitive.Content`, que Radix solo monta abierto.

### Arquitectura
- **`src/lib/gestos/`** (puro, probado): umbral de intención por distancia y ángulo (ratón 6 px, dedo 10 px, ≤35°), velocidad por ventana de 100 ms con los `timeStamp` de los eventos (no la hora de atenderlos), latigazo (≥0,45 px/ms) o posición proyectada (160 ms, umbral 35 %), goma elástica tipo iOS, rueda/trackpad horizontal (pausa de 180 ms), autodetección de perfil de entrada (táctil/ratón/híbrido por `any-pointer`/`any-hover`) y formato de pantalla, y la **sesión de borde** (`sesion-borde.ts`): la fuente del gesto (asa, sensor o toque en la orilla) publica las muestras y la cortina, al montarse, sigue al puntero 1:1 aunque haya nacido después del toque.
- **`useArrastrePanel`** (`src/hooks/use-arrastre-panel.ts`): Pointer Events para ratón, dedo y lápiz. **La captura solo se toma tras aceptar la intención**; los toques siguen siendo clics; el clic que sigue a un arrastre se suprime. Todo el movimiento vive en un MotionValue «cerradez» (0 abierto → 1 cerrado) → cero renders de React por píxel; «arrastrando» se marca en el DOM (`data-arrastrando`). La salida usa `usePresence` (funciona si la cortina se cierra desde el FAB, el orbe o un atajo). En paneles laterales, un `touchmove` no pasivo **limitado al panel y al gesto candidato** cancela solo el desplazamiento claramente horizontal. El transform es porcentual + margen medido (sin parpadeo al montar) y `none` en reposo (no atrapa hijos `fixed`).
- **`PanelCortina`** (`src/components/layout/trinity/panel-cortina.tsx`): armazón común de Zenith/Horizon/Logic — fondo que se oscurece en proporción a lo abierto (tocar fuera cierra), tirador en el canto interior, X común (`BotonCerrar`) en la esquina que mira al centro o donde la ponga el contenido (`usePanelCortina().cerrar`), `role="dialog"`, Escape, Tab atrapado y foco devuelto (`useModalA11y` con `initialFocus: "container"`), inclinación 3D sutil con movimiento completo.
  - Laterales: `touch-action: pan-y pinch-zoom`; se arrastran desde cualquier punto (salvo campos de texto, deslizadores y contenido con scroll horizontal). Estrecho: casi todo el ancho (queda una franja de fondo); md+: ancho fijo centrado.
  - Superior (Zenith): el arrastre solo nace en la cabecera y en el tirador inferior (`data-agarre-panel`), para no robar el scroll del chat; tocar el tirador no cierra (está sobre el final del chat).
- **Apertura desde el borde siguiendo al puntero:** dedo → `TrinityEdgeAccess` (Touch Events pasivos en `window`, banda de 28 px); ratón/lápiz → sensores de borde y asas (`useArrastreDesdeBorde`, captura en el propio sensor, que no tiene hijos). El sensor de la cortina abierta se aparta (`pointer-events: none`); con el dedo, tocar el borde con otra cortina abierta es «tocar fuera».
- **Dock (Anchor):** `DockDeslizable` con el mismo hook (lado abajo, solo desde su agarre); el carril lleva `touch-pan-x` y el arrastre de ratón del carril cede los movimientos verticales. Punto «estás aquí» compartido (`layoutId`) que se desliza entre iconos.
- **Globales:** `html { overscroll-behavior-x: none }` solo con puntero táctil (el borde es de Trinity; sin «atrás» por arrastre). El cinturón de modales pasa de `[class*="trinity-curtains_"]` a `[data-tirador-cortina]` y `[data-trinity-sensor]`.

### Movimiento y transiciones entre páginas, pasos y ventanas
- **Un solo nivel de movimiento** (`src/lib/movimiento/nivel.ts` + `useNivelMovimiento`): «completo» (resortes con 3D), «suave» (`data-perf` mid/eco: sin 3D) y «mínimo» (`prefers-reduced-motion` o Ajustes → Accesibilidad: solo fundidos; el arrastre directo sigue 1:1). Preajustes en `transiciones.ts`, curva CSS `--ss-curva-resorte` (`linear()` con respaldo bezier).
- **Páginas** (`TransicionRutas`, hermano en el layout raíz): tras una navegación en el cliente, las raíces de `#main-content` entran con la Web Animations API (sube desde atrás enderezando un `rotateX` de 5°, 420 ms). **Nunca** en la primera carga ni en la hidratación (el HTML del servidor se ve sin JS), nunca en rutas de consola, `fill: none` (sin transform residual que atrape piezas `fixed`), y solo fundido para raíces pequeñas. No se usa `template.tsx`: envolver `{children}` ya rompió el prerender de not-found (Adenda 137).
- **Coreografía del cromo:** `html[data-ss-navegando]` durante 620 ms → los elementos con `data-ss-coreo="logo"` (orbe de Aurora, emblema StarSeed, iconos de cabecera de las cortinas) giran en Y, el dock se asienta y el icono activo late; con las propiedades individuales `rotate`/`scale`/`translate`, que se suman al transform existente. El punto «estás aquí» del dock se desliza entre iconos (`layoutId`).
- **Diálogos** (`ui/dialog`, `ui/alert-dialog`): clase `ss-dialogo` (entran de canto desde atrás con resorte, salen hundiéndose; variante sin 3D en mid). **Pasos de asistentes** (`PasoAnimado` + `useDireccionPaso`): el paso entra girando desde el lado hacia el que se avanza y al retroceder se invierte (asistente de alta de la cuenta y guía de Aurora).

### Guías de introducción
- **Guía de Aurora** (`aurora-guide.tsx`): paso nuevo «Gestos naturales en cualquier pantalla» (tras Anchor) con texto y narración **según el dispositivo** (`bodyPorPerfil`/`sayPorPerfil` + `usePerfilDispositivo`: táctil, ratón o híbrido; cambia en vivo) y una **práctica real** (`demo-gestos-hibridos.tsx`): un mini panel Horizon que se abre tirando desde su borde y se cierra devolviéndolo, tocando fuera o con la X, con el MISMO motor de gestos; mano o cursor fantasma hasta que se prueba; logros anunciados por `aria-live`. La tarjeta se pasa deslizando (`useDeslizarPasos`: izquierda = siguiente), entra en 3D según la dirección, usa la X común y los títulos largos ya no se cortan en móvil. `starseed:open-guide` con `{ step }` vuelve a respetar el paso pedido (elegir el modo lo reiniciaba a 0).
- **Asistente de alta** (`onboarding-wizard.tsx`, paso «Guía de la red»): tarjeta `ResumenGestos` con cómo abrir y cerrar los menús en ESE dispositivo.

### Verificación (Chromium real, arnés con los componentes reales y los contextos simulados)
Guía: el paso híbrido se ve con su texto de ratón en escritorio y de dedo en móvil, y la práctica se completa (abrir desde el borde + cerrar con la X) con ratón y con toque real.
Transiciones: sin animación en la primera carga, entrada 3D al navegar y nada residual al terminar; logo girando; pasos en 3D que se enderezan; diálogo con su animación y su X; con movimiento reducido, solo fundidos.
Gestos, 27/27 escenarios: X por clic y por toque en las tres cortinas (disco ≥44 px en táctil), botones internos, Escape, clic/toque fuera, arrastre con ratón y con dedo, latigazo, arrastre corto que vuelve, scroll vertical intacto en Horizon y en el chat, rueda de trackpad, y apertura desde el borde siguiendo al dedo y al ratón (fondo a medio oscurecer a mitad de gesto). Pruebas de componente y del motor en `src/**/__tests__`.
