...
## 2026-09-06 — Adenda 227: olas 254–256 (modo ligero pulido · oído residente · BitNet estable)
- **Ola 254 — ligero pulido**: `starseed-ligero.sh` con heap 4096 por defecto, `construir --limpiar`, chequeo de disco ≥ 4 GB y estado con disco/RAM; las ocho rutas `/api/mando/*` que repetían su puerta (ramificacion, agentes, colas, orquestar, grafo, ajustes, entornos, contextos) pasan a `guardianMando(peticion)`; las nueve rutas del Mando responden 200 sin sesión en modo ligero y `/mando` pinta la ola activa y los agentes de la nube.
- **Ola 255 — oído residente**: el demonio usa `asr_stream_server` de VibeASR.cpp (modelos cargados una vez, stdin/stdout), sueño a los 5 min, carga hasta 240 s, presupuesto proporcional (60 s + 20 s/seg de audio, 120–360 s), `/status.asr` con `residente`/`cesiones`; cede la memoria del pool TTS (< 1200 MB y 10 s sin síntesis) y antes de cargar; V11 pasa la plantilla launchd a `ProcessType Interactive` (macOS ahogaba CPU/I/O en Background); medido: RTF 2,5 (20 s para 6,3 s de audio) tras V11+V12.
- **Ola 256 — BitNet estable** (repo astraura): `EconomicRouter._generar_local` habla con el llama-server BitNet nativo; causa raíz del segfault BLAS (`dequantize_row_i2_s` con prompts ≥ 32 tokens) → lanzar con `-ub 24 -b 24`; Ollama solo con `ASTRAURA_OLLAMA_RESPALDO=1`; subagente con presupuesto propio (150 s, 256 tokens); BitNet duerme a los 10 min (B5, en curso).
- **Lección de memoria**: oído 1,7 GB + BitNet 1,2 + backend ~1 + tts-server 0,9 + app de Claude ~0,8 → 3,3 GB comprimidos y 3,8 GB de swap; en Apple Silicon `vm_stat` usa páginas de 16 KB.
- **Regla de Alex**: todo lo ejecuta el orquestador; Claude diseña, supervisa en el Mando, verifica en localhost y aprueba. Detalle en `docs/adendas/adenda-227-ligero-oido-residente-bitnet-estable-olas-254-256-2026-09-06.md`.

## 2026-09-07 — Adenda 228: olas 257-276 (Mando ampliado · aprendizaje continuo 1.58 · claves por medio)
- **Aprendizaje continuo 1.58**: planificador asíncrono con cinco agentes (Curador 30 min, Evaluador 60 min, Cronista 60 min operativos; Entrenador/Desplegador «esperando fábrica»), endpoints `/api/aprendizaje/agentes` y `/api/aprendizaje/procesos`, pausar/reanudar/ejecutar; nunca despierta el BitNet. Curador atómico (`leer_mes`/`reescribir_mes` bajo cerrojo, `.bak`, temporal+`os.replace`, solo con duplicados; rotación de `logs.md`/`cronica.md` a 2 MB). El fondo NO cae a Ollama con BitNet dormido (`meta.omitido`; Ollama solo con `ASTRAURA_OLLAMA_RESPALDO=1`); servidor adoptado cuenta como vivo y `ensure_server(marcar=False)` no anula «dormir». M7 «Ramificación 1.58» en Procesos y M8 pestaña «Aprendizaje»; procesos con envoltorio `{procesos:[…]}` y corpus por HTTP. Verificado en la Mac: 5 agentes, corpus 1486 turnos de `cognition` + 1 de `astraura_prime` (3,8 MB), puntuación 30 (latencia 95–184 s).
- **Claves por medio (P9–P9E, M9, M9B)**: capa `CLAVES_POR_PROVEEDOR`, `claves_de`, `clave_activa`, `agotar_clave`, `estado_claves`, sufijos `_2…_9`, huellas sha256 (nunca valores), cableada en `llamar_llm`/sondas/opencode. 429 → 1 h, 402/cuota → 24 h, aviso único; **sonda ligera `GET /models`** (la de generación quemaba el cupo diario y tres 429 dejaban xKiro 24 h fuera); puerta pytest que mira el árbol de trabajo. M9 Flota honesta con 15 proveedores y «Por conseguir (solo Alex)»; M9B clasifica con las claves presentes (`clavesPresentes`) y un solo contador «agotados».
- **Oficina 3D (O1–O3B)**: modelo puro `src/lib/mando/oficina.ts` (seres por escritor/revisor/agente 1.58/personalidad/proceso/BitNet, ADN determinista `derivarAdn`, 7 salas, xp/nivel, `fusionarGenoma` nunca baja), servidor `/api/mando/oficina` con genomas en `starseed_memory_root/mando/oficina/genomas.json` y `exportar-predeterminado`; pestaña «Oficina 3D» con `OficinaSeres` y ficha del ser. Verificado: 7 salas, 28 seres.
- **Almacenamiento y Drive (A1–A3)**: `/api/mando/almacenamiento` (df, regenerables con lista blanca, DriveFS, espejo rsync sin `--delete` a `My Drive/StarSeed_Memory_Root/neurona-<host>`, swap honesta, «aliviar»); tarjetas Almacenamiento/Google Drive/Swap + «Disco libre» en cabecera; `POST /ceder` del demonio (dormir 20 s). Espejo real de 2,1 MB.
- **Commits pendientes (C1–C3)**: `/api/mando/publicaciones` (commits sin publicar del OS y Astraura por ola, diffstat, delante/detrás, árbol limpio, enjambre escribiendo); publicar SOLO con `STARSEED_LOCAL=1` y `PUBLICAR` escrito; pestaña «Commits pendientes» con diálogo de confirmación; paquete con ref. Verificado: 122 commits del OS delante de `2755cd2`.
- **Voces (V1–V5B)**: `voz-mando.ts`, `/api/mando/voces` + `voz-del-mando.tsx`, `panel-voces.tsx`, pestaña «Voces» y `?pestana=`; `TIMBRES` pasa a `timbres-catalogo.ts` sin «use client» (arreglo de build) y test V5B que lo impide.
- **Forja fase 4 (I1A–I1C, K1)**: demonio `POST /clonar` (multipart, consentimiento, WAV 24 kHz mono 3–20 s, `.rvq`), `GET /clones`, `DELETE`, `clon:true` en `/tts`; cierre del path traversal por `personality` (`CLON_TIMBRE_RE`, `rutaRef`); `clonacion.ts` y subpestaña «Clonar».
- **Verificación (Q1c–Q1E)**: `scripts/verificar-neurona.mjs` (checks HTTP, umbrales, comparación, `--voz/--oido/--bitnet`, `--help`, `starseed_memory_root/verificaciones/ultimo.json`); primera corrida real 92/100.
