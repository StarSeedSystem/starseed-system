...
(async () => {
  // Launch Chrome with flags to avoid the remote debugging popup
  ...
  const browser = await chromium.launchPersistentContext(userDataDir, {
    ...
    args: [
      ...
      '--hide-scrollbars',
      '--mute-audio',
      '--remote-allow-origins=*' // Try to allow all origins to connect via DevTools protocol
    ]
  });
  ...
  // After submit, wait a bit and then look for any button that might appear (like a modal)
  await page.waitForTimeout(2000);
  // Check if we are still on login
  let currentUrl = page.url();
  console.log('After submit URL:', currentUrl);
  if (currentUrl.includes('/login')) {
    // Look for buttons that might be in a modal
    ...
    for (let i = 0; i < buttons.length; i++) {
      ...
      // If the button text suggests continuing, click it
      if (text && (text.includes('Continuar') || text.includes('Entrar') || text.includes('Iniciar Sesión') || text.includes('Aceptar') || text.includes('Accept') || text.includes('OK'))) {
    ...
    currentUrl = page.url();
    console.log('After clicking button URL:', currentUrl);
  }
