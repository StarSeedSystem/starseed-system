...
async function mergeBrains() {
  ...
  // Group by owner
  const brainsByOwner: Record<string, any[]> = {};
  ...
  for (const [owner, userBrains] of Object.entries(brainsByOwner)) {
    ...
    const accountBrains = userBrains.filter((b: any) => b.scope === 'account' && !b.scope_ref);
    ...
    accountBrains.sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());

    const primaryBrain = accountBrains[0];
    const duplicates = accountBrains.slice(1);

    let mergedIncludes: any = { ...primaryBrain.includes };
