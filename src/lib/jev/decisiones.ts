...
// SOLO funciones puras: construir validar, leer y normalizar. Sin red, sin
// disco, sin claves. La llamada HTTP vivirá en una ruta de servidor futura.
...
function validarTexto(v: unknown, donde: string): string {
    if (typeof v !== "string" || !v.trim()) {
        throw new Error(`Pregunta «${donde}»: instructions debe ser texto no vacío`);
    }
    return v;
}
    ...
    for (const nombre of nombres) {
        ...
        if (p.type === "choice") {
            ...
            if (!c || typeof c !== "object" || Array.isArray(c)) {
                throw new Error(`Pregunta «${nombre}»: choice exige criteria objeto`);
            }
            const claves = Object.keys(c);
            if (claves.length === 0) {
                throw new Error(`Pregunta «${nombre}»: choice necesita al menos una opción`);
            }
            if (claves.length > MAX_OPCIONES) {
                throw new Error(`Pregunta «${nombre}»: máximo ${MAX_OPCIONES} opciones`);
            }
            for (const k of claves) {
                if (typeof c[k] !== "string" || !c[k]) {
                    throw new Error(`Pregunta «${nombre}»: opción «${k}» sin significado`);
                }
            }
            continue;
        }
        if (p.type === "score") {
            ...
            if (!Array.isArray(c) || c.length === 0) {
                throw new Error(`Pregunta «${nombre}»: score exige criteria array ordenado`);
            }
            if (c.length > MAX_OPCIONES) {
                throw new Error(`Pregunta «${nombre}»: máximo ${MAX_OPCIONES} niveles`);
            }
            for (const nivel of c) {
                if (typeof nivel !== "string" || !nivel) {
                    throw new Error(`Pregunta «${nombre}»: nivel de score vacío`);
                }
            }
            continue;
        }
...
export function leerRespuesta(json: unknown): Record<string, Decision> {
    const raiz = (json ?? {}) as { answers?: Record<string, Record<string, unknown>> };
    ...
    for (const nombre of Object.keys(answers)) {
        ...
        const probabilidades =
            a.probabilities && typeof a.probabilities === "object" && !Array.isArray(a.probabilities)
                ? (a.probabilities as Record<string, number>)
                : {};
        let valor: string | number = numero(a.noul);
        if (tipo === "choice") {
            valor = typeof a.choice === "string" ? a.choice : (Object.keys(probabilidades)[0] ?? "");
        } else if (tipo === "score") {
            valor =
                typeof a.score === "string"
                    ...
                    : typeof a.score === "number"
                      ? a.score
                      : (Object.keys(probabilidades)[0] ?? "");
        }
        out[nombre] = {
            tipo,
            valor,
            probabilidades,
            confianza: numero(a.confidence),
        };
    }
    return out;
}

/* ─── Utilidades de umbral y preguntas estándar ───────────────────────────── */
...
export function decisionesDeModeracion(p: PublicacionModerable): Record<string, Pregunta> {
    ...
    return {
        permitida: {
            type: "noul",
            instructions: "¿Cumple esta publicación las normas de la comunidad?",
        },
        etiquetas: {
            type: "choice",
            instructions: "¿Qué categoría describe mejor esta publicación?",
            criteria: criteriaEtiquetas,
        },
        prioridad_feed: {
            type: "score",
            instructions: "¿Qué prioridad debería tener en el feed?",
