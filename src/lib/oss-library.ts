...
const OSS_LIBRARY_EXT: OssOption[] = [
  ...
  { id: "audiomorphic", name: "Audiomorphic", category: "creation", description: "Visualizador de consciencia del propio ecosistema StarSeed: traduce el sonido del micrófono en GEOMETRÍA VIVA — una espiral fractal (Zn+1 = Zn·k·e^iψ) gobernada por el «Tratado de Unificación Armónica», con tres pilotos automáticos (Deriva · Armónico · Génesis) y geometría sagrada en resonancia (Flor de la Vida, Espiral Áurea, Onda Cuántica, Toroide). ESTADO EN EL OS: **PORTADO Y FUNCIONAL, NO ES UN ENLACE** — su motor corre nativo dentro del OS (src/lib/audiomorphic/), en dos superficies: la APP completa en /audiomorphic (sin tour, sin login, sin planes: nada estaba realmente bloqueado, los «tiers» eran solo UI) y una CAPA DE FONDO del sistema con TRANSPARENCIA REAL (canvas con alfa; el original era `alpha:false` + estela que pinta negro ⇒ opaco por diseño). El micrófono se concede con un clic, nunca al cargar. HONESTIDAD: el modo VR/AR NO está portado — su motor 3D (@react-three/xr v6 + postprocessing v3) exige React 19 + R3F v9 y el OS va con React 18 + R3F v8; para VR/AR se abre la app original o el APK/DMG de sus releases.", license: "Repo del ecosistema StarSeed (StarSeedSystem)", url: "https://github.com/StarSeedSystem/Audiomorphic-AR-app", oss: true, maintained: true, defaultIntegrated: true, tags: ["visualizador", "audio", "geometria-sagrada", "fondo", "portado", "nativo", "starseed"] },
