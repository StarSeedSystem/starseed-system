/**
 * API route del puente Hermayone (servidor de la neurona = esta Mac).
 *
 * Recibe un mensaje del usuario desde un chat de Aurora (reenviado por
 * `hermayone-bridge.ts` cuando Hermayone está activa) y lo entrega a la
...
import { writeHermayoneReply } from "@/lib/aurora/hermayone-bridge";

const HERMES_WS = process.env.HERMAYONE_HERMES_WS || "ws://localhost:8787";
  ...
  return new Promise<string | null>((resolve) => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (data?.type === "hermayone-reply" && typeof data.text === "string") {
          clearTimeout(timer);
          try { ws.close(); } catch { /* noop */ }
          resolve(data.text);
        }
...
export async function GET() {
  return NextResponse.json({ ok: true, bridge: "hermayone", status: "listening" });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const ok = await writeHermayoneReply({
      convId: body.convId,
      userId: body.userId,
      text: reply,
      clientId: `hermayone-reply-${body.clientId}`,
    });
