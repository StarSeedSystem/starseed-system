...
export function LiveDataWidget() {
    ...
    const updated = useMemo(() => new Date().toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit", second: "2-digit" }), [data]);
                ...
                !loading && data ? (
                    <div className="flex items-center justify-between gap-2 text-[10px] font-semibold text-muted-foreground/60 min-w-0">
                        <span className="inline-flex items-center gap-1.5 min-w-0">
                            ...
                            <span className="truncate">{data.length} señales · refresco 3s</span>
                        </span>
                        ...
                        {showSpark && (
                            <div className="flex-1 min-h-0 rounded-xl border border-border/40 bg-white/[0.02] p-2.5 flex flex-col">
                                <div className="flex items-center justify-between gap-2 mb-1 min-w-0">
                                    ...
                                    <span className="text-[11px] font-black tabular-nums shrink-0" style={{ color: lead.color ?? "#06b6d4" }}>{lead.display ?? lead.value}{lead.unit ? ` ${lead.unit}` : ""}</span>
                                </div>
