  ...
  return (
    ...
    >
      ...
      <div className="rounded-xl border border-[#7fb8ff]/25 bg-[#7fb8ff]/[0.05] p-3 space-y-2.5">
        ...
        {omniRoute !== "local" && <LocalEngineInstaller />}
...
/**
 * INSTALADOR DEL MOTOR LOCAL por dispositivo (Adenda 80). Detecta el sistema y
 * las capacidades del equipo, recomienda la variante (BF16/Q8_0/Q4_K_M) y da la
 * instalación de UNA LÍNEA (o el paso honesto en plataformas sin soporte aún).
 * El daemon resultante escucha SOLO en 127.0.0.1:4444, arranca con la sesión
 * del usuario y procesa la voz 100 % en el dispositivo.
 */
function LocalEngineInstaller() {
  ...
  const det = useMemo(() => {
    ...
    const label =
      os === "mac" ? "macOS" : os === "windows" ? "Windows" : os === "android" ? "Android" : os === "ios" ? "iOS / iPadOS" : "Linux";
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-sky-500/25 bg-sky-500/[0.06] p-2.5 text-[11px] text-sky-100/90">
      ...
      <p className="leading-snug text-sky-100/70">
        ...
        sondea tu hardware y elige sola la exacta). El motor escucha solo en
        127.0.0.1:4444, arranca con tu sesión, se duerme a los 10 min y se actualiza
        cada 7 días con la red de la Librería. Privacidad total: la voz se procesa aquí.
      </p>
      ...
      {det.os === "mac" && (
        <p className="leading-snug text-sky-100/50">
          Pega la línea en Terminal (⌘+Espacio → «Terminal»). Necesita Node 18+ y git;
          el script instala cmake solo si falta (con Homebrew) y te pide permiso.
        </p>
      )}
      {det.os === "linux" && (
        <p className="leading-snug text-sky-100/50">
          Pega la línea en tu terminal. Necesita Node 18+, git y cmake (el script usa
          apt si falta). El servicio queda como unidad de usuario de systemd.
        </p>
      )}
      {det.os === "windows" && (
        <p className="leading-snug text-sky-100/60">
          Windows (beta, sin servicio automático): instala{" "}
          <a className="underline underline-offset-2" href="https://nodejs.org" target="_blank" rel="noreferrer">Node 18+</a>{" "}
          y git, y en PowerShell:{" "}
          <code className="rounded bg-black/40 px-1 font-mono text-[10px]">
            git clone --depth 1 https://github.com/StarSeedSystem/starseed-system; cd starseed-system/native/astraura-voice; node install.mjs --no-service
          </code>{" "}
          — luego deja <code className="rounded bg-black/40 px-1">node daemon.mjs</code> en el arranque.
        </p>
      )}
      {(det.os === "android" || det.os === "ios") && (
        <p className="leading-snug text-sky-100/60">
          En {det.label} todavía no hay instalación nativa: la voz funciona igualmente
          con la nube gratis automática, y este mismo equipo puede usar el motor de
          otra neurona tuya cuando esté en línea.
        </p>
      )}
    </div>
  );
