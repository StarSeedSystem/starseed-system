-- ════════════════════════════════════════════════════════════════════════════
-- Adenda 149 (parche de acoplamiento) — EXCLUIR role='pending' del CENSO server-side.
-- ----------------------------------------------------------------------------
-- La aprobacion de ingreso a grupos (migracion 20260805220000) modela una
-- solicitud de ingreso como una fila os_memberships con role='pending' (aun NO
-- miembro). El censo AUTORITATIVO de gov_resolve_proposal (Adenda 146) contaba
-- TODAS las filas os_memberships por group_slug, incluyendo las pendientes, lo
-- que inflaba el denominador de quorum-por-% y podia contar a un pendiente como
-- elegible. El JS (membership.ts) ya excluye 'pending'; este parche alinea el
-- censo SQL (la fuente de verdad del sellado).
--
-- SOLO cambia DOS consultas (marcadas), re-creando gov_scope_member_ids (camino
-- federado reach) y gov_resolve_proposal (censo de ambito unico) con
-- `and m.role is distinct from 'pending'`. Todo lo demas es IDENTICO a la
-- migracion 20260805190000 (verificado por diff: unica diferencia = las 2 lineas
-- de filtro). Idempotente (create or replace) + reversible (rollback restaura las
-- funciones de A146 verbatim). NO toca proposals_guard ni gov_resolve_slug.
-- ════════════════════════════════════════════════════════════════════════════
  ...
  select distinct u from (
    ...
    select m.user_id::text as u
      ...
      where m.group_slug = public.gov_resolve_slug(p_ref)
        ...
        and m.role is distinct from 'pending'
    union all
